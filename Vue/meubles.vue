            ...
            <div class="col">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col">#</th>
                            <th scope="col">description</th><th scope="col">id</th><th scope="col">type</th><th scope="col">nom</th> <!-- <th scope="col">Action</th>-->
                    ...
                    <tbody>
                        <tr v-for="(item, index) in Liste" :key="index" @click="selectItem(item)">
                            <th scope="row">{{ index + 1 }}</th>
                             <td>{{ item.description }}</td><td>{{ item.id }}</td><td>{{ item.type }}</td><td>{{ item.nom }}</td> <!-- <td>{{ item.temperatureF }}</td> -->
                            <td>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form @submit.prevent="submitForm">
                        <div class="mb-3"><label for="description" class="form-label">description</label><input type="text" class="form-control" id="description" v-model="selectedItem.description"></div><div class="mb-3"><label for="id" class="form-label">id</label><input type="text" class="form-control" id="id" v-model="selectedItem.id"></div><div class="mb-3"><label for="type" class="form-label">type</label><input type="text" class="form-control" id="type" v-model="selectedItem.type"></div><div class="mb-3"><label for="nom" class="form-label">nom</label><input type="text" class="form-control" id="nom" v-model="selectedItem.nom"></div>
                        <!-- <div class="mb-3">
...
export default {
    ...
    methods: {
        ...
        async updateElement() {
            try {
                const response = await fetch(`http://localhost:8080/meubles/${this.selectedItem.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
